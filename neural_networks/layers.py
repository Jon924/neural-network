...
class FullyConnected(Layer):
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        dX = np.dot(dZ, self.parameters["W"].T)
        dW = np.dot(X.T, dZ)
        db = np.sum(dZ, axis=0).reshape((1, self.n_out))
        #db = np.dot(np.ones((1, dZ.shape[0])), dZ).reshape((1, self.n_out))
...
class Conv2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        ### BEGIN YOUR CODE ###
        self.cache['X'] = X
        colX =  im2col(X, kernel_shape, self.stride, self.pad)
        #print(colX[0].shape, W.shape)
        inner = kernel_height*kernel_width*in_channels
        #print(W.T.reshape((out_channels, inner)).shape)
        out = (W.T.reshape((out_channels, inner))@colX[0]) + b.T
        #print(out.shape)
        out = col2im(colX[0], X, W.shape, self.stride, colX[1])
        #print(out.shape)
        #out = out
